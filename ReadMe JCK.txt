Check vars with names(responses). After munging, should be 245 vars
ending with "TotSal_f".

After running project, can use View(ScalesItems) to see csv of items and scales. Command also in eda.R
...
2.	Launch ProjectTemplate (Following text generated by p;; via FastKeys:
...
eda.R	Calculates scale psychometrics and item descriptives and outputs to text files in output folder. 
cfa.R for confirmatory factor analysis
LinearRegression.R for linear regression
